...
# to control the amount of symultaneous request
from threading import Thread
from Queue import Queue

# amount of concurent requests (10 sets of 9x images sizes)
concurrent = 10*9

def doWork():
    while True:
        imageInfo = q.get()
        bytesize, (width,height) = getJpgSizesFromUrl(imageInfo.url)
        doSomethingWithResult(imageInfo.url, imageInfo.testSize, bytesize, (width,height))
        q.task_done()
...
def doSomethingWithResult(url, testSize, bytesize, (width,height)):
    #print url, bytesize, (width,height)
    if bytesize is not None:
        #sys.stdout.write(str(row["EANHotelID"]) + '|' + row["Caption"] + '|' + str(row["DefaultImage"]) + '|')
        # current size type working on
        sys.stdout.write(url +'|')
        sys.stdout.write(testSize+'|')
...
class ImageCheck(object):
    def __init__(self, url, testSize):
        self.url = url
        self.testSize = testSize
        return
    
# generate the queue of threads
...
with con:
    ...
    for row in rows:
        ...
        for size_to_test in list_to_test:
            ...
            try:
               q.put( ImageCheck(imageURL.strip(),size_to_test) )
               q.join()
